...
public final class PhoneMasker {
    public static final String MASK = "XXXXXXXXX";

    // Includes ordinary spaces, non-breaking spaces and common directional marks used by diallers.
    ...
    public static boolean containsSavedNumber(CharSequence text, SavedNumberRepository repository) {
        if (text == null || text.length() == 0 || repository == null) {
            return false;
        }
        ...
        while (matcher.find()) {
            ...
            if (digitCount >= 7 && digitCount <= 16 && repository.isSavedNumber(candidate)) {
                return true;
            }
        }
        return false;
    ...
    public static String redactSavedNumbers(CharSequence text, SavedNumberRepository repository) {
        ...
        while (matcher.find()) {
            String candidate = matcher.group();
            int digitCount = digitsOnly(candidate).length();
            if (digitCount >= 7 && digitCount <= 16 && repository != null
                    && repository.isSavedNumber(candidate)) {
